...
class Repetition extends Model
{
    ...
    /**
     * Returns an array of occurrences for until
     * a given date
     */
    public function nextOccurrences(Carbon $after, Carbon $endAt): ?array
    {
        $occurrences = [];

        while($after->lessThan($endAt))
        {
            $date = $this->nextOccurrence($after);
            $after = $date;
            $occurrences[] = $date->toISOString();
        }
